...
-----------------
[rewrite_local]
^https:\/\/hqapp.faw.cn\/fawcshop\/collect-public\/v1\/score\/addScore? url script-request-header https://raw.githubusercontent.com/zhanghaifei1997/Quantumult_X/main/HqCookie.js
...
[task_local]
5 0 * * * https://raw.githubusercontent.com/evilbutcher/Quantumult_X/master/check_in/hzh/hzh.js, tag=红旗智联
...
const $ = new API("hq", true);
const ERR = MYERR();
$.cookie = $.read("evil_hqCookie");
$.usertoken = $.read("evil_hqUserToken");
...
!(async () => {
  ...
  if ($.cookie != undefined && $.usertoken != undefined) {
    ...
    $.notify("红旗智联", "", "❌ 请先获取Cookie");
  }
})()
  .catch((err) => {
    if (err instanceof ERR.ParseError) {
      $.notify("红旗智联", "❌ 解析数据出现错误", err.message);
    } else if (err instanceof ERR.EventError) {
      $.notify("红旗智联", "❌ 请尝试重新获取Cookie", err.message);
    } else {
      $.notify(
        "红旗智联",
        "❌ 出现错误",
...
function checkin() {
  ...
  const headers = {
    ...
    "User-Agent": `HQZL_Ultimate_iOS/3.10.0 (iPhone; iOS 15.0.2; Scale/2.00)`,
...
function checkinfo() {
  const url2 = `https://hqapp.faw.cn/fawcshop/mall/v1/apiCus/getUserInfo`;
  const headers2 = {
    // Origin: `https://campaign.huazhu.com`,
    Cookie: $.cookie,
	aid: '1426952704488189954',
...
function showmsg() {
  ...
  if ($.data.isSign != null && $.data.isSign == true) {
    $.notify("红旗智联", "今日已签到🎉", `累计签到${count}天！`);
  } else if ($.data.isSign != null && $.data.isSign == false) {
    point = $.data.point;
    $.notify("红旗智联", "签到成功🎉", `获得${point}积分，累计签到${count}天！`);
  }
}
...
function getCookie() {
  if (
    ...
    $.log(cookie);
    $.write(cookie, "evil_hqCookie");
    const usertoken = $request.headers["User-Token"];
    $.log(usertoken);
    $.write(usertoken, "evil_hqUserToken");
    $.notify("红旗智联", "", "获取签到Cookie成功🎉",cookie);
    $.notify("红旗智联", "", "获取签到usertoken成功🎉",usertoken);
  }
}
