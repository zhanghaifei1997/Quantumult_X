...
let cookiesArr = [];
const activityList = [{
	'activityId': 'af2b3d56e22d43afa0c50622c45ca2a3',
...
async function getReward() {
	...
	const body =
		`{"groupType":5,"configCode":"${$.moduleBaseInfo.configCode}","itemId":1,"eid":"${$.eid}","fp":"${$.fp}"}`;
...
async function doTask() {
	...
	const body =
		`{"groupType":${$.oneTask.groupType},"configCode":"${$.moduleBaseInfo.configCode}","itemId":"${$.item.itemId}","eid":"${$.eid}","fp":"${$.fp}"}`;
...
async function getActivity() {
	const url =
	`https://jdjoy.jd.com/module/task/v2/getActivity?configCode=${$.activityID}&eid=${$.eid}&fp=${$.fp}`;
	const headers = {
		'Accept': `application/json, text/plain, */*`,
		'content-type': 'application/json;charset=utf-8',
		'Origin': `https://prodev.m.jd.com`,
