...
async function run() {
	try {
		...
		let config = [
			...
			{
				configCode: '761d289b16d74713bf6cee8462ca0e76',
				configName: '我爱520-2'
			},
			...
			if ($.task.showOrder) {
				...
				for (let i = 0; i < $.taskList.length; i++) {
					...
					if ($.task.showOrder == 1) {
						...
						console.log(
							`${$.oneTask.cardName} ${0 == $.oneTask.result ? "开卡得" + $.oneTask.rewardQuantity + "京豆" : 1 == $.oneTask.result ? "领取" + $.oneTask.rewardQuantity + "京豆" : 3 == $.oneTask.result ? "其他渠道入会" : "已入会"}`
							)
						if ($.oneTask.result == 0) await statistic(
						...
						if ($.oneTask.result == 1 || $.oneTask.result == 0) await getReward(
						...
						$.outActivity = false
						let name =
							`${1 == $.oneTask.groupType ? "关注并浏览店铺" : 2 == $.oneTask.groupType ? "加购并浏览商品" : 3 == $.oneTask.groupType ? "关注并浏览频道" : 6 == $.oneTask.groupType ? "浏览会场" : "未知"}`
						let msg = `(${$.oneTask.finishCount}/${$.oneTask.taskCount})`
						let status =
							`${$.oneTask.finishCount >= $.oneTask.taskCount && '已完成' || "去" + (1 == $.oneTask.groupType ? "关注" : 2 == $.oneTask.groupType ? "加购" : 3 == $.oneTask.groupType ? "关注" : 6 == $.oneTask.groupType ? "浏览" : "做任务")}`
						...
						if ($.oneTask.finishCount < $.oneTask.taskCount) {
							...
							for (n = 2; c-- && !$.outActivity; n++) {
								...
								let q = 3
								for (m = 1; q--; m++) {
									if ($.oneTasks == '') await getActivity(item.configCode, item.configName, $
										.oneTask.groupType)
									if ($.oneTasks) break
								}
								if ($.oneTasks) {
									...
									if ($.oneTasks.item.itemId != $.oneTask.item.itemId && $.oneTasks.finishCount <
										$.oneTasks.taskCount) await doTask(
				...
				if ($.task.showOrder == 2) {
					...
					if ($.taskInfo.rewardStatus == 1) await getReward(
					...
					if (typeof res == 'object') {
						if (res.success == true && res.data.pass == true) {
							if (flag == 0) {
								...
								$.taskInfo = res.data.moduleBaseInfo || res.data
									.moduleBaseInfo || []
							} else if (flag == -1) {
								$.taskInfo = res.data.moduleBaseInfo || res.data
									.moduleBaseInfo || {}
			...
			try {
				if (err) {
					...
					if (typeof res == 'object') {
						if (res.success == true) {
							console.log(
								`领奖成功:${flag == 1 && $.taskInfo.rewardFinish || $.oneTask.rewardQuantity}京豆`
								)
							$.bean += Number($.oneTask.rewardQuantity)
						} else {
							console.log(`${res.errorMessage}`)
...
async function getUA() {
	...
	let arr = await getBody($.UA, 'https://prodev.m.jd.com/mall/active/3q7yrbh3qCJvHsu3LhojdgxNuWQT/index.html')
	$.fp = arr.fp
	await getEid(arr)
}
...
class JdJrTdRiskFinger {
	f = {
		...
		doNotTrackKey: function(a) {
			this.options.excludeDoNotTrack || (a.push(this.getDoNotTrack()), y = y + ",'track':'" + this
				.getDoNotTrack() +
				"'");
			return a
		},
		...
		pluginsKey: function(a) {
			this.isIE() ? (a.push(this.getIEPluginsString()), y = y + ",'plugins':'" + t(this
					.getIEPluginsString()) +
				"'") : (a.push(this.getRegularPluginsString()), y = y + ",'plugins':'" + t(this
...
var JDDSecCryptoJS = JDDSecCryptoJS || function(t, u) {
	...
	x.UUID = w.extend({
		generateUuid: function() {
			for (var g = "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".split(""), m = 0, a = g.length; m <
				a; m++)
				switch (g[m]) {
					case "x":
	...
	var y = v.enc = {},
		n = y.Hex = {
			...
			parse: function(g) {
				for (var m = g.length, a = [], b = 0; b < m; b += 2) a[b >>> 3] |= parseInt(g.substr(b, 2),
					16) <<
					24 - b % 8 * 4;
			...
			for (var l = this._keySchedule = [], h = 0; h < a; h++)
				if (h < c) l[h] = b[h];
					...
					h % c ? 6 < c && 4 == h % c && (q = x[q >>> 24] << 24 | x[q >>> 16 & 255] << 16 | x[
						q >>> 8 & 255] << 8 | x[q & 255]) : (q = q << 8 | q >>> 24, q = x[q >>>
						24] <<
						24 | x[q >>> 16 & 255] << 16 | x[q >>> 8 & 255] << 8 | x[q & 255], q ^= m[
							h /
							c | 0] << 24);
					l[h] = l[h - c] ^ q
		...
		_doProcessBlock: function(A, y) {
			for (var n = this._hash.words, e = n[0], f = n[1], r = n[2], k = n[3], g = n[4], m = 0; 80 >
				...
				a = 20 > m ? a + ((f & r | ~f & k) + 1518500249) : 40 > m ? a + ((f ^ r ^ k) +
					1859775393) : 60 > m ? a + ((f & r | f & k | r & k) - 1894007588) : a + ((f ^
					r ^
					k) - 899497514);
				g = k;
				k = r;
...
var _CurrentPageProtocol = "https:" == document.location.protocol ? "https://" : "http://",
	...
	_CurrentPageUrl = function() {
		...
		var u = t.indexOf("?");
		0 < u && (_url_query_str = t.substring(u + 1), 500 < _url_query_str.length && (_url_query_str = _url_query_str
			.substring(
...
var td_collect = new function() {
	...
	this.obtainLocal = function(n) {
		...
		try {
			window.localStorage && null !== window.localStorage && 0 !== window.localStorage.length && (e
				.localStorage =
				window.localStorage.getItem("3AB9D23F7A4B3C9B"))
		} catch (k) {}
		try {
			window.sessionStorage && null !== window.sessionStorage && (e.sessionStorage = window
				.sessionStorage[
					"3AB9D23F7A4B3C9B"])
		} catch (k) {}
		try {
	...
	this.collect = function() {
		...
		if (g) {
			...
			try {
				...
				l && (r.wuv = k.getParameter(l.UNMASKED_VENDOR_WEBGL), r.wur = k.getParameter(l
		...
		try {
			f.cookie = navigator.cookieEnabled, f.localStorage = !!window.localStorage, f.sessionStorage = !!
				window.sessionStorage, f.globalStorage = !!window.globalStorage, f.indexedDB = !!window
				.indexedDB
		} catch (D) {}
		e.ss = f;
	...
	this.collectSdk = function(n) {
		try {
			...
			if (null != r && void 0 != r && "" != r) try {
				var k = JSON.parse(r),
					g = (new Date).getTime();
				null != k && void 0 != k.t && "number" == typeof k.t && (12E5 >= g - k.t && void 0 != k
					.tk &&
					null != k.tk && "" != k.tk && k.tk.startsWith("jdd") ? (e.deviceInfo.sdkToken = k
						.tk,
						f = !0) : void 0 != k.tk && null != k.tk && "" != k.tk && (e.deviceInfo
						.sdkToken =
						k.tk))
			} catch (m) {}
			r = !1;
			e.deviceInfo.isJdApp ? (e.deviceInfo.clientVersion = navigator.userAgent.split(";")[2], (r = 0 < e
				.compareVersion(
	...
	this.getJdBioToken = function(n) {
		...
		window._bioDeviceCb = function(f) {
			try {
				...
				null != r.data.token && void 0 != r.data.token && "" != r.data.token && (e.deviceInfo
					.sdkToken =
					r.data.token)
			} catch (k) {
